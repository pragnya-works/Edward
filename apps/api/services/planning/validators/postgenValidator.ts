...
function resolveValidationFramework(
  ...
  const hasViteEntrypoints = files.has("src/main.tsx") && files.has("src/App.tsx");

  if (
    framework === GENERATED_OUTPUT_FRAMEWORK.VITE_REACT &&
    hasNextEntrypoints &&
    !hasViteEntrypoints
  ) {
  ...
  if (
    framework === GENERATED_OUTPUT_FRAMEWORK.NEXTJS &&
    hasViteEntrypoints &&
    !hasNextEntrypoints
  ) {
