...
Own the streaming chat-session runtime: prompt assembly, agent loop execution,
parser event handling, sandbox/tool side effects, and final message
persistence.
...
3. Parser events trigger sandbox file writes, dependency install, command, and
   web-search tools.
