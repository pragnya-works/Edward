...
import { disconnectContainerFromNetwork, TIMEOUT_DEPENDENCY_INSTALL_MS } from '../utils.sandbox.js';
...
export async function buildAndUploadUnified(sandboxId: string): Promise<BuildResult> {
  ...
  try {
    ...
    const pnpmCheck = await execCommand(container, ['which', 'pnpm'], false, 5000);
    if (pnpmCheck.exitCode !== 0) {
      logger.warn({ sandboxId }, 'pnpm not found in container, installing globally');
      ...
      if (pnpmInstall.exitCode !== 0) {
        logger.error({ sandboxId, stderr: pnpmInstall.stderr.slice(-300) }, 'Failed to install pnpm in container');
        await disconnectContainerFromNetwork(containerId, sandboxId);
        return {
          success: false,
          buildDirectory: null,
          error: `Cannot install pnpm: ${pnpmInstall.stderr || pnpmInstall.stdout}`.slice(0, 300),
          previewUploaded: false,
          previewUrl: null,
        };
      }
    ...
    if (!scaffoldedFramework) {
      const detectedFramework = await detectFrameworkFromPackageJson(container, sandboxId);
      if (detectedFramework) {
        scaffoldedFramework = detectedFramework;
        sandbox.scaffoldedFramework = detectedFramework;
        await saveSandboxState(sandbox);
        logger.info({ sandboxId, detectedFramework }, 'Auto-detected framework from package.json');
      }
    }
    ...
    if (nodeModulesMissing) {
      logger.warn({ sandboxId }, 'node_modules missing before build, running pnpm install');
      ...
      if (installResult.exitCode !== 0) {
        ...
        return {
          success: false,
          buildDirectory: null,
          error: `Dependency installation failed (exit ${installResult.exitCode}): ${installResult.stderr || installResult.stdout}`.slice(0, 500),
          previewUploaded: false,
          previewUrl: null,
        };
      }
    ...
    {
      const mergeResult = await mergeAndInstallDependencies(containerId, requestedPackages, sandboxId);
    ...
    if (userId && chatId) {
      ...
      await deleteFolder(previewPrefix).catch(err =>
        logger.warn({ err, sandboxId, previewPrefix }, 'Failed to clean old preview files (non-fatal)')
      );
    }
    ...
    if (userId && chatId) {
      await invalidatePreviewCache(userId, chatId).catch(err =>
        logger.warn({ err, sandboxId }, 'CloudFront invalidation failed (non-fatal)')
      );
    }
...
async function detectFrameworkFromPackageJson(
  ...
  try {
    ...
    logger.warn({ error, sandboxId }, 'Failed to detect framework from package.json');
    return undefined;
  }
}
