...
function validateCommandArgs(command: string, args: string[]): void {
  if (args.length > MAX_ARG_COUNT) {
    throw new Error(`Too many arguments (${args.length}), max ${MAX_ARG_COUNT}`);
  }
  ...
  for (const arg of args) {
    ...
    if (!resolved.startsWith(CONTAINER_WORKDIR) && !resolved.startsWith('/home/node')) {
...
export async function executeSandboxCommand(
    ...
    logger.info({ sandboxId, command: params.command, args: params.args }, 'Executing sandbox command');
    ...
    try {
        ...
        const maxOut = params.command === 'cat' ? MAX_OUTPUT_CAT : MAX_OUTPUT_DEFAULT;
        result.stdout = truncateOutput(result.stdout, maxOut);
        result.stderr = truncateOutput(result.stderr, maxOut);

        return result;
    } catch (error) {
        logger.error({ error, sandboxId, command: params.command, args: params.args }, 'Sandbox command execution failed');
        throw error;
    }
}
