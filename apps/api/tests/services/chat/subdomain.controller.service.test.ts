...
describe("updateChatSubdomain", () => {
  ...
  it("commits DB updates before routing registration, then removes old routing", async () => {
    const { updateChatSubdomain } = await import(
      "../../../services/previewRouting/subdomainUpdate.service.js"
    );
    ...
    await updateChatSubdomain(req, res);
...
describe("checkSubdomainAvailability", () => {
  ...
  it("validates chat ownership before checking availability", async () => {
    ...
    await checkSubdomainAvailability(req, res);
