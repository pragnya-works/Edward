...
const ROOT_ANCESTOR_GUIDE_DEPTHS: number[] = [];
      ...
      <div className="py-2">
        {nodes.map((node) => (
          <FileTreeItem
            ...
            node={node}
            depth={0}
            ancestorGuideDepths={ROOT_ANCESTOR_GUIDE_DEPTHS}
          />
        ))}
      </div>
...
const FileTreeItem = memo(function FileTreeItem({
  ...
  const nextAncestorGuideDepths = useMemo(
    function createNextAncestorGuideDepths() {
      return [...ancestorGuideDepths, depth];
    },
            ...
            >
              ...
              {node.children.map((child) => (
                <FileTreeItem
                  ...
                  node={child}
                  depth={depth + 1}
                  ancestorGuideDepths={nextAncestorGuideDepths}
                />
              ))}
            </m.div>
          )}
