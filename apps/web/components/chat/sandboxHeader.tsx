...
export function SandboxHeader() {
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2 border-b border-border/40 bg-background/50 backdrop-blur-md shrink-0">
      <div className="flex items-center gap-2">
        <div className="h-7 w-7 rounded-lg bg-primary/10 flex items-center justify-center">
          <Code2 className="h-4 w-4 text-primary" />
        </div>
        <div className="flex flex-col">
          <span className="text-[11px] font-bold tracking-tight">Workspace</span>
            ...
            buildStatus === BuildStatus.FAILED ? (
              <div className="flex items-center gap-1">
                {buildStatus === BuildStatus.FAILED ? (
                  ...
                  <RefreshCw className="h-2 w-2 text-amber-500 animate-spin" />
                )}
                <span
                  className={cn(
      ...
      <div className="flex items-center gap-2">
        <Tabs
          ...
          className="w-auto"
        >
          <TabsList className="h-7 p-0.5 bg-foreground/[0.03] border-none shrink-0">
            <TabsTrigger
              value="code"
              className="h-6 px-3 text-[10px] gap-1.5 font-semibold transition-all data-[state=active]:bg-background data-[state=active]:text-primary"
            >
              Code
            </TabsTrigger>
            <TabsTrigger
