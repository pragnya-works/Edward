...
export function getRepoInputValidationError(rawValue: string): string | null {
  const validation = validateGithubRepositoryInput(rawValue);
  if (validation.valid) {
    return null;
  }
...
export function getBranchNameValidationError(rawValue: string): string | null {
  const validation = validateGithubBranchName(rawValue);
  if (validation.valid) {
    return null;
  }
  return validation.message;
